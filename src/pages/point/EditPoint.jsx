...
import { useGetPoint, useEditPoint } from '@/api/point';
...
import { Save, MapPin, Loader2, Info, Network, NotebookPen, Edit3 } from "lucide-react";

function EditPoint({ pointId, open, setOpen, userId, userRole }) {
  const { data: point, isLoading: isFetchingPoint } = useGetPoint(pointId);
  ...
  useEffect(() => {
    if (point) {
      reset({
        ...
        active: point.active,
        name: point.name,
        max_subscription: point.max_subscription,
              ...
              <div className="flex items-center justify-between border border-stone-100 rounded-2xl h-14 px-5 bg-stone-50/50 mt-2 transition-all hover:bg-stone-50">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-stone-400">تفعيل/تعطيل النقطة</span>
                  </div>
                </div>
                <Switch
                  checked={!!activeValue}
